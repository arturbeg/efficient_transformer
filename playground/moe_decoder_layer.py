...
class MoeDecoderLayer(nn.Module):
    ...
    # self, d_model, heads, dropout = 0.1, is_lm = True, mixing = "none", is_cuda = True

    def __init__(self, d_model, heads, num_experts, dropout=0.1, is_lm=True, mixing="none", is_cuda=True, noisy_gating=True, k=1):
        ...
        self.k = k
        # instantiate experts
        self.experts = nn.ModuleList([DecoderLayer(d_model=d_model, heads=heads, dropout=dropout, is_lm=is_lm, mixing=mixing, is_cuda=True) for i in range(self.num_experts)])
        ...
        self.normal = Normal(torch.tensor([0.0]).to(devce=self.device), torch.tensor([1.0]).to(device=self.device))
    ...
    def forward(self, x, train=True, loss_coef=1e-2):
        ...
        expert_tuple_outputs = [self.experts[i](expert_inputs[i]) for i in range(self.num_experts)]
